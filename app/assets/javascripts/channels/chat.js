var my_id = 0;
var chatroom_list = new Object();
var notification_channel;
var ping_timer = new Object();
...
function startChat(friend_id,received){
  ...
  chatroom_list[room_id] = App.cable.subscriptions.create(
    {"channel":"ChatChannel","my_id":my_id,"friend_id":friend_id}, {
    connected: function() {
    },
  ...
  $(document).on('keypress', '[data-behavior~=chat_post-'+room_id+']', function(event) {
    if (event.keyCode === 13) {
      ...
      try{
        ...
        chatInput.val('');
        connectionTimer(room_id, "接続できませんでした。相手がオフラインのため、メッセージが届いていない可能性があります。");
      } catch (e){
...
function createCommentBaloon(room_id,data) {
  ...
  if(link_flg) {
    ...
    splitted_mess = splitted_mess.map(escape_html)
  ...
  if(my_id == data['message']['user_id']) {
    ...
    $("<div>", {class: 'icon'}).appendTo(comment_left);
...
function printSystemMessage(room_id, message){
  var div = $("<div>", {class: 'chatroom-sysmessage',text:message});
...
function subscribeNotificationChannel() {
  ...
  notification_channel = App.cable.subscriptions.create(
    {"channel":"ChatChannel","my_id":my_id,"friend_id":0}, {
    connected: function() {
